...
tui-test.generated_config_contract() {
	...
	rg -q 'setup\.inner_phase_reached secure_boot' "$repository_root/setup.sh"
	...
	rg -q 'install -d -m 2755 -o root -g systemd-journal' "$repository_root/setup.sh"
	rg -q 'systemctl enable gdm.service' "$repository_root/setup.sh"
	rg -q 'shared/default-x-display-manager select gdm3' "$repository_root/setup.sh"
	rg -q 'dpkg-reconfigure -f noninteractive gdm3' "$repository_root/setup.sh"
	rg -q 'rm -f -- /run/dbus/system_bus_socket' "$repository_root/setup.sh"
